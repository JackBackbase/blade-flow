...
class LandingActionHandlerIT extends BaseIntegrationIT {

    @Test
    @WithFlowAnonymousUser
    void postInteraction_withValidData_returnResult() throws Exception {
        var emailAddress = "test@email.invalid";
        var interactionInstance = createInteractionInstanceInCaseStore(
            SmeCaseDefBuilder.getInstance().firstName("b").lastName("b").dateOfBirth(LocalDate.parse("1970-01-01"))
                .email(emailAddress).isRegistrar(true).build(),
            TestConstants.SME_ONBOARDING_LANDING, UUID.randomUUID().toString());
        ...
        mockMvc.perform(post(
            ...
            TestConstants.SME_ONBOARDING_LANDING_DATA)
            ...
            .andExpect(jsonPath("$.body.caseId").isNotEmpty())
            .andExpect(jsonPath("$.body.email", Matchers.is(emailAddress)));
    }
